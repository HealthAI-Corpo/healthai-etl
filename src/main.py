...
def run_test_insert():
    ...
    try:
        ...
        existing_user = session.query(Utilisateur).filter_by(email="jordan.test@healthai.com").first()
        ...
        if not existing_user:
            ...
            session.commit()
            print(f"Utilisateur '{new_user.prenom}' créé (ID: {new_user.id_utilisateur})")
        else:
